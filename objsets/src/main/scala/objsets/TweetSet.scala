...
class Empty extends TweetSet {
  ...
  def mrAcc(current: Tweet) = current
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mrAcc(current: Tweet) = {
    val newCurrent = if(elem.retweets > current.retweets) elem else current
    removeElem mrAcc newCurrent
  }

  def descendingByRetweet = {
    val hottest = mostRetweeted
    new Cons(hottest, (this remove hottest) descendingByRetweet)
  }
