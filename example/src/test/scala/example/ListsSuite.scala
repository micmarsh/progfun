...
class ListsSuite extends FunSuite {
  ...
  test("throw some negatives and duplicates into max") {
    assert(max(List(-9,0,0,2)) === 2)
  }
  
  test("get zeroes for empty sum, exception for empty max") {
    assert(sum(List()) === 0)
    intercept[Exception] {
      (max(List()))
    }
