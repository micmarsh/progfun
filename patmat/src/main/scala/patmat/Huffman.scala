...
object Huffman {
  ...
  def weight(tree: CodeTree): Int = tree match {
    case Fork(l,r,cs, result) => result
    case Leaf(c, result) => result
  }

  def chars(tree: CodeTree): List[Char] = tree match {
    case Fork(l,r,result,w) => result
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def accumulator(acc: Map[Char,Int], item: Char) = {
      val int = acc get item
      if (int == None) acc + (item -> 1)
      else acc + (item -> (int.get + 1))
    } 
    val starter = Map[Char, Int]()
    val map = chars.foldLeft(starter)(accumulator)
    map.toList
  }
  /**
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    val sorted = freqs.sortWith((pair0:(Char, Int), pair1:(Char, Int)) => pair0._2 < pair1._2)
    sorted.map((pair:(Char, Int)) => Leaf(pair._1, pair._2))
  }
   ...
   */
  def until(terminator: List[CodeTree] => Boolean, combiner: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]) = {
    def loop(trees: List[CodeTree]): List[CodeTree] =
      if (terminator(trees)) trees
      else loop(combiner(trees))
    loop(trees)
  }
  ...
  def createCodeTree(chars: List[Char]): CodeTree = 
    until(singleton, combine)(makeOrderedLeafList(times(chars.toList))).head
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def makeSingleChar(tree: CodeTree, bits: List[Bit]): (Char, List[Bit]) = 
      tree match {
        ...
        case Fork(left, right, full, weight) => {
          if (bits.head == 0) makeSingleChar(left, bits.tail)
    ...
    def loop(acc: List[Char], bits: List[Bit]): List[Char] = 
       if(bits.isEmpty) acc
         ...
         val (char, newBits) = makeSingleChar(tree, bits)
         loop(char::acc, newBits)
       }
    
    loop(Nil, bits).reverse
   }
